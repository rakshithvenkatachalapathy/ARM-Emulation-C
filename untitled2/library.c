...
void cache_direct_mapped_stats(struct cache_direct_mapped *dm) {
    float hr = ((float) dm->hits / (float) dm->refs);
    float mr = ((float) dm->misses / (float) dm->refs);
    printf("dm(%d) ", dm->size);
    printf("refs: %d ", dm->refs);
    printf("hits: %d ", dm->hits);
    printf("misses: %d ", dm->misses);
...
unsigned int cache_lookup(struct cache_state *cstate, unsigned int addr) {
    ...
    for (int i = 0; i < 4; ++i) {
        data[j++] = cache_lookup_direct_mapped(&cstate->dm_array[i], addr);
    }
    for (int i = 0; i < 4; ++i) {
        data[j++] = cache_lookup_full_associative(&cstate->fa_array[i], addr);
    }
    for (int i = 0; i < 4; ++i) {
        data[j++] = cache_lookup_set_associative(&cstate->sa_array[i], addr);
    }
    for (int i = 0; i < 12; i++) { //sanity check
        if (data[0] != data[i])
            flag = false;
    }
    if (!flag)
